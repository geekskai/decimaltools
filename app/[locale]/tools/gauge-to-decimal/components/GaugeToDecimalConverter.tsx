  ...
  return (
    <section className="rounded-3xl border border-cyan-500/30 bg-gradient-to-br from-cyan-500/18 via-blue-500/8 to-purple-500/18 p-4 shadow-lg md:p-6">
      <div className="space-y-5 md:space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 md:items-stretch">
          <div className="rounded-2xl border border-cyan-500/20 bg-slate-950/45 p-3.5 md:p-5">
            <div className="flex h-full flex-col gap-4">
              <div>
                <label className="mb-2 block text-sm font-semibold text-cyan-100">{t("search_label")}</label>
                <input
                  value={searchInput}
                ...
                />
                <p className="mt-2 text-xs text-slate-400 md:text-sm">{t("search_helper")}</p>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="h-12 w-full rounded-2xl border border-cyan-500/30 bg-slate-950/80 px-4 text-sm text-white transition-all duration-300 focus:border-cyan-400 focus:outline-none focus:ring-4 focus:ring-cyan-500/20 md:text-base"
              ...
              <div>
                <p className="mb-2 text-sm font-semibold text-slate-200 md:mb-3">{t("quick_search_title")}</p>
                      ...
                      <div className="flex gap-2 overflow-x-auto pb-1">
                        {group.values.map((quick) => (
                          <button
                            ...
                            className={`shrink-0 rounded-full border px-2 py-1 text-xs transition-all duration-300 md:text-sm ${
                              activeGauge === quick
            ...
            {result.ok ? (
              <div className="space-y-3">
                <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
                  ...
                  <p className="mt-1.5 text-sm font-bold text-white md:text-base">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-2xl border border-white/10 bg-slate-950/50 p-4">
                    ...
                    <p className="mt-1.5 text-sm text-slate-100 md:text-base">{result.value.inchesLabel}</p>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-slate-950/50 p-4">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-400 md:text-xs md:tracking-[0.16em]">
                      {t("millimeters_label")}
                    </p>
                    <p className="mt-1.5 text-sm text-slate-100 md:text-base">{result.value.millimetersLabel}</p>
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-white/10 bg-slate-950/50 p-4">
                  ...
                  <p className="mt-1.5 text-sm text-slate-100 md:text-base">{result.value.toleranceLabel}</p>
                  <p className="mt-1.5 text-xs text-slate-300 md:text-sm">
                    {t("tolerance_range_label")}: {result.value.toleranceRangeInchesLabel}
                  </p>
                  <p className="mt-1 text-xs text-slate-300 md:text-sm">{result.value.toleranceRangeMmLabel}</p>
                </div>
                ...
                <div className="rounded-2xl border border-white/10 bg-slate-950/50 p-4">
                  ...
                  <div className="mt-2 space-y-1.5">
                    {result.value.gaugeMatches.map((row) => (
                      <p key={row.standard} className="text-xs text-slate-200 md:text-sm">
                        {t(`standards.${row.standard}`)}: {row.inches.toFixed(4)}" / {row.millimeters.toFixed(3)} mm
                      </p>
                    ))}
                  </div>
                </div>
        ...
        <details className="group rounded-2xl border border-cyan-500/20 bg-slate-950/45 p-3.5 md:p-5" open>
          <summary className="cursor-pointer list-none pr-5 text-sm font-semibold text-cyan-100 marker:content-none md:text-base">
            {t("lookup_title")}
          </summary>

          <div className="mt-3 space-y-3">
            <div>
              <label className="mb-2 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-300 md:text-sm">
              ...
              <input
                ...
                className="h-11 w-full rounded-2xl border border-cyan-500/30 bg-slate-950/70 px-4 text-sm text-white placeholder-slate-500 transition-all duration-300 focus:border-cyan-400 focus:outline-none focus:ring-4 focus:ring-cyan-500/20"
              />
            </div>

            <div className="overflow-hidden rounded-2xl border border-white/10">
              <div className="grid grid-cols-[0.9fr_1fr_1fr_auto] gap-2 bg-slate-900/70 px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-300 md:px-4 md:text-xs">
                <span>{t("lookup_gauge_col")}</span>
                <span>{t("lookup_inch_col")}</span>
                <span>{t("lookup_mm_col")}</span>
                <span>{t("lookup_action_col")}</span>
              </div>
              <div className="max-h-64 overflow-y-auto bg-slate-950/50">
            ...
            {lookupData.queryGauge !== null && lookupData.rows.length !== 1 ? (
              <p className="text-xs text-amber-200 md:text-sm">{t("lookup_not_found")}</p>
            ) : null}
          </div>
        </details>
      </div>
